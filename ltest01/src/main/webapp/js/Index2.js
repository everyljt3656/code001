...
function delById(id){
	...
	if(confirm("确定要删除该客户?")){
		location.href="/ltest01/customer/delete/"+id;
	}
}
...
function add() {
    if ($("#custName").val() == "") {
        //layer.tips('不能为空', '#custName');
    	$("#adderror").text("客户名称不能为空");
    	$("#custName").val("");
    	$("#custName").focus();
        return;
    }
    ...
    if ($("#custAddress").val() == "") {
    	...
    	$("#adderror").text("客户地址不能为空");
    	$("#custAddress").val("");
    	$("#custAddress").focus();
        return;
    }
    if ($("#nickname").val()==''&&!confirm("确定不关联用户角色")) {
    	return;
    }
    
    $("#adderror").text("");//清空错误信息
    ...
    var formdata = {
        ...
        custLinkUser:$("#nickname").val(),
    }
    $.ajax({
        url: "/ltest01/customer/add",
        timeout: 300000,
        dataType: "json",
        type: "post",
        data: formdata,
        success: function (data) {
            ...
            //load(curr);
            alert("添加成功!!");
            location.href="/ltest01/customer/tocenter";
        }
    })
}
